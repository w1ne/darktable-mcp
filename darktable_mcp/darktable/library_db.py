...
This module deliberately bypasses darktable's Lua API for one reason only:
``view_photos`` must be headless. Spawning the darktable GUI to answer a
library query is unacceptable for an MCP tool whose purpose is to return
data to the AI client. ``darktable-cli`` is the headless darktable binary
but it does not load the user's library, so it cannot browse it. That
leaves library.db as the only headless source of library state.

The file is opened read-only via SQLite URI so we coexist with a running
darktable. Writes go through the Lua API (or a future plugin) instead.
"""
